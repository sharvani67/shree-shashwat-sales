  ...
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      ...
      const formattedInvoices = staffInvoices.map(transaction => ({
        ...
        transactionType: transaction.TransactionType // Keep for reference
      }))
    ...
    if (filters.status !== 'all') {
      filtered = filtered.filter(invoice => {
        ...
        if (filterStatus === 'overdue') {
          return isOverdue(invoice.dueDate)
        }
      ...
      <div className="staff-pending-invoices-container">
        ...
        <div className="staff-pending-invoices-filters">
          ...
          <div className="staff-pending-invoices-status-filters">
            ...
            <button
              className={`staff-pending-invoices-filter-btn ${filters.status === 'active' ? 'staff-pending-invoices-filter-active' : ''}`}
          ...
          <div className="staff-pending-invoices-list">
            {filteredInvoices.map((invoice, index) => (
              <div 
                ...
                className={`staff-pending-invoice-card ${expandedInvoice === invoice.voucherId ? 'staff-pending-invoice-expanded' : ''}`}
              >
                {/* Card Header */}
                <div 
                ...
                >
                  ...
                  <div className="staff-pending-invoice-header-right">
                    <span className={`staff-pending-invoice-status-badge ${getStatusColor(invoice.status)}`}>
                      {getStatusIcon(invoice.status)}
                      {invoice.status}
                      {isOverdue(invoice.dueDate) && <span className="staff-pending-invoice-overdue-indicator"> (Overdue)</span>}
                    </span>
                ...
                <div className="staff-pending-invoice-basic-info">
                  ...
                  <div className="staff-pending-invoice-info-row">
                    ...
                    <div className="staff-pending-invoice-info-item">
                      ...
                      <span className={`staff-pending-invoice-info-value ${isOverdue(invoice.dueDate) ? 'staff-pending-invoice-due-overdue' : ''}`}>
