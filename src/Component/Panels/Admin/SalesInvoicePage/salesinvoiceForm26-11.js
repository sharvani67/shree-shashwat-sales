...
invoiceData.items.forEach((item, index) => {
  console.log(`Item ${index + 1}:`, {
    ...
    batch: item.batch,        // Should be like "tec001"
    batch_id: item.batch_id,  // Should be like 314 (numeric ID)
...
<Form.Select
  ...
  onChange={(e) => {
    ...
    if (batch) {
      setItemForm(prev => ({
        ...prev,
        batch: batchNumber, // This is batch_number like "dell002"
        batch_id: batch.id, // ✅ FIX: This should be the actual ID from batches table (312, 314, etc.)
        price: batch.selling_price
      }));
      console.log('✅ Batch ID set:', batch.id, 'Batch Number:', batchNumber); // Debug log
    } else {
...
>
  ...
  {batches.map((batch) => (
    <option key={batch.id} value={batch.batch_number}>
      {batch.batch_number} (Qty: {batch.quantity} | ID: {batch.id})
    </option>
  ))}
</Form.Select>
  </Col>
